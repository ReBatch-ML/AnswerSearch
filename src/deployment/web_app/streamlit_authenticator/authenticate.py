...
class Authenticate:
    ...
    def __init__(
        self, credentials: dict, cookie_name: str, key: str, cookie_expiry_days: int = 30, preauthorized: list = None
    ):
        """
            ...
            if len(new_email) and len(new_username) and len(new_name) and len(new_password) > 0:
                if new_username not in self.credentials['usernames']:
                    if new_password == new_password_repeat:
                        if preauthorization:
                            preauthorized = [
                                fnmatch(new_email.lower(), pattern) for pattern in self.preauthorized['emails']
                            ]
                            print(preauthorized)
                            print("\n")
                            if any(preauthorized):
