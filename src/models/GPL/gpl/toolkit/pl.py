...
class PseudoLabeler(object):
    ...
    def retokenize(self, texts):
        ...
        features = self.retokenizer(
            texts, padding=True, truncation='longest_first', return_tensors="pt", max_length=self.max_seq_length
        )
        decoded = self.retokenizer.batch_decode(
            features['input_ids'], skip_special_tokens=True, clean_up_tokenization_spaces=True
        )
        return decoded
